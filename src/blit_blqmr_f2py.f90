...
!> @brief Solve sparse system AX = B (multiple RHS, OpenMP parallel) - F2PY interface
!>
!> @param[in]  nblock   Block size for OpenMP parallelism

subroutine blqmr_solve_real_multi_omp(n, nnz, nrhs, nblock, Ap, Ai, Ax, B, X, &
                                       maxit, qtol, droptol, pcond_type, &
                                       flag, iter, relres)
    use blit_precision
    use blit_blqmr_real
    ...
    !f2py intent(in) n, nnz, nrhs, nblock, maxit, pcond_type
    !f2py intent(in) qtol, droptol
    !f2py intent(in) Ap, Ai, Ax, B
    !f2py intent(out) X, flag, iter, relres
    !f2py depend(n) Ap
    ...
    integer, intent(in) :: n, nnz, nrhs, nblock, maxit, pcond_type
    real(kind=Kdouble), intent(in) :: qtol, droptol
    integer, intent(in) :: Ap(n+1), Ai(nnz)
    real(kind=Kdouble), intent(in) :: Ax(nnz), B(n, nrhs)
    real(kind=Kdouble), intent(out) :: X(n, nrhs), relres
    ...
    ! Prepare preconditioner and solve with OpenMP block parallelism
    call BLQMRPrep(qmr, Ap_local, Ai_local, Ax_local, nnz_local)
    call BLQMRSolveOMP(qmr, Ap_local, Ai_local, Ax_local, nnz_local, X_local, B_local, nrhs)
...
end subroutine blqmr_solve_real_multi_omp
...
!> @brief Solve complex sparse system AX = B (multiple RHS, OpenMP parallel) - F2PY interface
!>
!> @param[in]  nblock   Block size for OpenMP parallelism

subroutine blqmr_solve_complex_multi_omp(n, nnz, nrhs, nblock, Ap, Ai, Ax, B, X, &
                                          maxit, qtol, droptol, pcond_type, &
                                          flag, iter, relres)
    use blit_precision
    use blit_blqmr_complex
...
end subroutine blqmr_solve_complex_multi_omp
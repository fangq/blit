...
class TestSystemFixtures:
    ...
    @staticmethod
    def small_system():
        """5x5 sparse test system from Fortran test program."""
        n, nnz = 5, 12
        ...
        return {"Ap": Ap, "Ai": Ai, "Ax": Ax, "b": b, "x": x_expected, "n": n, "nnz": nnz}
...
class TestBackendDetection(unittest.TestCase):
    ...
    def test_get_backend_info(self):
        ...
        self.assertIn(info['backend'], ['fortran', 'native'])
...
class TestBLQMRSolve(unittest.TestCase):
    ...
    def test_tolerance_levels(self):
        ...
        for tol in [1e-4, 1e-8, 1e-12]:
            with self.subTest(tol=tol):
                ...
                if result.converged:
                    self.assertLessEqual(result.relres, tol * 100)
    ...
    def test_no_preconditioner(self):
        ...
        result = blqmr_solve(
            d['Ap'], d['Ai'], d['Ax'], d['b'],
            use_precond=False, tol=1e-6, maxiter=500
        )

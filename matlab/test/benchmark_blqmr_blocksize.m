...
II = zeros(16*nelem, 1); JJ = zeros(16*nelem, 1);
...
for e = 1:nelem
    ...
    for i = 1:4
        for j = 1:4
            cnt = cnt + 1;
            II(cnt) = idx(i); JJ(cnt) = idx(j); VV(cnt) = Ae(i,j);
        end
    end
end
...
for i = 1:size(B,2)
    bn = norm(B(:,i));
    if bn > 0; r = max(r, norm(A*X(:,i)-B(:,i))/bn); end
end
end